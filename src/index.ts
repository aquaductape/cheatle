...
function getEliminationWords(
  ...
  return {
    ...
    message: `Choose "${result.foundWord}" in order to get rid of the ${result.matches.length} words that contain "${result.pattern}" character pattern (such as ${matchedSentence})`,
  };
}
